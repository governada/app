...
- **Worktree isolation for feature work.** NEVER create feature branches in the main `governada-app` checkout. All feature work MUST happen in a worktree (`npm run worktree:new -- <name>` or `git worktree add .claude/worktrees/<name> -b feat/<name> origin/main`). The main checkout stays on `main` at all times. Enforced by the branch guard hook — edits on feature branches in the main checkout are blocked. Only hotfixes (with `ALLOW_MAIN_EDIT=1`) bypass this.

- Sandbox-safe default: `npm run worktree:new -- <name>` creates `.claude/worktrees/<name>` inside the repo instead of a sibling directory.
...
- Prefer the repo's `npm run ...` entrypoints for diagnostics, GitHub auth, CI watching, failed-log tails, deploy verification, Inngest registration, and worktree setup. The Mac-first workflow should not require PowerShell.
- Recommended persistent approvals: `npm run worktree:new`, `npm run worktree:sync`, `npm run session:doctor`, `npm run session:guard`, `npm run gh:auth-status`, `npm run auth:repair`, `npm run ci:watch`, `npm run ci:failed`, `npm run pre-merge-check`, `npm run deploy:verify`, `npm run inngest:register`, `git add`, `git add -A`, `git commit -m`, `git push`, `git fetch origin main`, `git worktree add`, and `gh api repos/governada/governada-app/pulls`.
- Do **not** persist approvals for broad shells or interpreters like bare `zsh`, `bash`, `node`, `python`, `git`, or `gh`.
...
- Authoritative repo bootstrap files: `.mcp.json`, `.claude/settings.local.json`, `.env.local`, `package.json`, `scripts/lib/runtime.js`, `scripts/set-gh-context.js`, `scripts/gh-auth-status.js`, and `scripts/repair-gh-auth.mjs`.
...
- Hotfixes: direct on main. Features: `npm run worktree:new -- <name>`
...
- Hooks and ship commands are Node/sh-first. Use `scripts/set-gh-context.js` for repo-scoped GitHub CLI context.
...
**Session start**: `npm run worktree:sync` auto-runs where configured and HARD BLOCKS if behind origin/main with a dirty tree. Fix: stash+rebase or commit+rebase before any work (including planning). Reading stale files produces plans that reference deleted/renamed code.
...
**Before pushing**: the behind-main hook blocks `git push` if behind origin/main. Fix: `git rebase origin/main`.
...
Legacy `scripts/*.ps1` files are compatibility shims. Canonical desktop-agent automation should use the Node/sh hooks and `npm run ...` entrypoints above.
