...
1. **Worktree check**: Run `git rev-parse --show-toplevel` and check if `.git` is a directory (main checkout) or file (worktree). If you're in the main checkout and the task requires feature work, STOP and create a fresh worktree with `powershell -ExecutionPolicy Bypass -File scripts/launch-feature-codex.ps1` or `powershell -ExecutionPolicy Bypass -File scripts/new-worktree.ps1 <name>`, or tell the user to relaunch with `claude --worktree <name>`. Do NOT create branches in the main checkout. Hotfixes (with `ALLOW_MAIN_EDIT=1`) are the only exception.
