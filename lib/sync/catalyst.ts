...
function toFundRow(f: CatalystFund) {
  return {
    ...
    amount: f.amount != null ? Math.round(f.amount) : null,
...
function toProposalRow(p: CatalystProposal) {
  return {
    ...
    yes_votes_count: p.yes_votes_count != null ? Math.round(p.yes_votes_count) : null,
    no_votes_count: p.no_votes_count != null ? Math.round(p.no_votes_count) : null,
    abstain_votes_count: p.abstain_votes_count != null ? Math.round(p.abstain_votes_count) : null,
    amount_requested: p.amount_requested != null ? Math.round(p.amount_requested) : null,
    amount_received: p.amount_received != null ? Math.round(p.amount_received) : null,
    currency: p.currency,
    problem: p.problem,
    solution: p.solution,
    ...
    unique_wallets: p.unique_wallets != null ? Math.round(p.unique_wallets) : null,
    yes_wallets: p.yes_wallets != null ? Math.round(p.yes_wallets) : null,
    no_wallets: p.no_wallets != null ? Math.round(p.no_wallets) : null,
    synced_at: new Date().toISOString(),
  };
}
...
async function flushCampaigns(
  ...
  const rows = Array.from(campaigns.values()).map(({ campaign: c, fundId }) => ({
    ...
    amount: c.amount != null ? Math.round(c.amount) : null,
    fund_id: fundId,
